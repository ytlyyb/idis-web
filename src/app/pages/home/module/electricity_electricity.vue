...
export default {
  ...
  mounted() {
    this.pieAmmeter();
    this.ammeterStatistics();
  },
  methods: {
    ...mapActions({
      ElectricMeterStatistics: "Electric_meter_statistics", //电表统计
    }),
    ammeterStatistics() {
      this.ElectricMeterStatistics({}).then((res) => {
        if (res.success == 1) {
          this.abnormalCommCount = res.data.abnormalCommCount;
          this.normalCommCount = res.data.normalCommCount;
          this.isvHeight = this.$parent.$parent.$parent.Heights7 || this.$parent.$parent.$parent.Size7
